...
### 🔧 ISSUE-011 — PM 未自动重试 W4 任务（撞限 abort 后烂尾）【部分修复】
...
| **等级** | P1 严重 |
| **模块** | PM Agent 重试机制 |
| **报告时间** | 2026-06-29 10:44 |
| **报告人** | 用户提醒 |
| **修复时间** | 2026-06-29 10:47（部分修复：手动重派 + 加固 SOUL.md §5.7）|
| **修复人** | PM |
| **状态** | 🔧 部分修复（手动重派 W4 成功 + 加固重试规则，**待验证**=后续 task 是否真的自动重试）|

**症状**：
- W4 E-004 派单时间：6/29 08:17 (runId `0008f36c`)
- dev 跑了 ~2.5 小时无产出（期间 W4 commit 不存在）
- dev 进程已不在（撞限后 OpenClaw retry 失败 → abort）
- **PM 未自动重新派单**，任务完全烂尾
- 用户 10:44 提醒：“M3 5h 配额已重置，为何不继续？”
- **PM 主动重派 W4（10:47）后才发现问题**

**根因**：
1. ❌ PM §5 速率限制规则只定义了“**重试逻辑**”，**没有自动执行机制**
2. ❌ 每次 spawn 后，PM 只 yield 等结果，**没设超时 / 没设 fallback cron**
3. ❌ subagent abort 后，PM 接收不到信号（除非用户提醒）
4. ❌ PM SOUL.md 虽记录了 §5 规则，但**仅手动**，未集成到 spawn 流程里

**经验教训**：
- 🚨 **仅写文档不等于执行**——规则要能自动跑起来才是有数的规则
- 🚨 **PM 是**服务**对子任务的**，不能被动等结果——必须主动监控
- 🚨 **每次 spawn 都看做“可能 abort”**——**默认加** +1h cron 重试备份

**永久方案**（已实施）：
1. ✅ PM SOUL.md §5.7 新增：“强制自动重试机制”
   - 提供“自动 retry 实现模板”（copy-paste 能执行）
   - 5 级跳级重试逻辑
   - PM 主动监控检查清单
2. ✅ W4 已手动重派（runId `682ee63b-8ef5-4038-82c0-1a3ae2a3074b`）
3. ⏳ **待验证**：后续 task 是否真的自动带重试。**找机会追加个 verification**：下一个新 spawn task 时，验证 §5.7 是否被执行。

**起令**：2026-06-29 10:47 PM 受用户提醒后加固规则 + 手动重派 W4。**希望以后不再需要用户提醒。**
...
### ✅ ISSUE-012 — cron 触发撞 memory flush 时无 exec 工具,日报补录延迟 12 分钟【已修复】

| 字段 | 值 |
|---|---|
| **Issue ID** | ISSUE-012 |
| **等级** | P2 一般 |
| **模块** | PM Agent 工具链 / cron 任务 |
| **报告时间** | 2026-06-29 22:05 |
| **报告人** | PM 自报(cron 触发时发现)|
| **修复时间** | 2026-06-29 22:20 |
| **修复人** | PM |
| **状态** | ✅ 已修复(cron 补录机制 + PM 兜底流程已规范化)|

**症状**:
- 2026-06-29 22:00 cron:07970c65 自动触发日报生成
- 当前会话处于 pre-compaction memory flush 窗口
- `write` 工具被锁定,只能写 `memory/2026-06-29.md`
- 项目目录 `docs/03-management/daily-reports/2026-06-29.md` 写入失败
- 日报延后 12 分钟(22:17 用户 /compact 解锁后才补录)

**根因**:
1. ❌ cron 任务提示要求"用 exec + PowerShell Set-Content 写入",但本次会话**无 exec 工具**(只有 read/write)
2. ❌ PM 没有"memory flush 窗口兜底流程",默认假设会话工具齐全
3. ❌ write 锁死时,PM 没有自动 fallback 到 "memory 文件 + 后续补录" 模式

**兜底方案**(已实施):
1. ✅ 日报全文已写入 `memory/2026-06-29.md` §"今日开发日报全文",完整保留
2. ✅ 22:17 用户 /compact 后 PM 自动补录到项目目录
3. ✅ 新增日报 README 提示:"日报可能有 ≤15 分钟延迟(若撞 flush)"
4. ✅ 加固 PM SOUL.md §6:memory flush 兜底流程

**教训**:
- 🚨 **write 工具被锁时不要硬扛**,优先落 memory,后续补录
- 🚨 **cron 任务要预设"工具缺失"场景**,不能假设所有工具都可用
- 🚨 **用户 /compact 可以解除 flush 锁**,告诉用户这个逃生通道
