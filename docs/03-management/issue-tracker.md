...
### ISSUE-034 — 国内 Android 应用商店上架 [V1.3 主线 (Phase 1 纯本地)]

| 字段 | 值 |
|---|---|
| **Issue ID** | ISSUE-034 |
| **关联决策** | [ADR-005](../02-architecture/04-adr/ADR-005-android-cn-only.md) + [ADR-008](../02-architecture/04-adr/ADR-008-phase1-local-only-cloud-deferred.md) |
| **替代 ISSUE** | ISSUE-025 (Google Play 暂缓) |
| **等级** | **P0 V1.3 主线** |
| **模块** | 发布 / 国内 Android 应用商店 |
| **状态** | 📋 计划中 (V1.3 Phase 1 = 纯本地版) |
| **决策日期** | 2026-07-15 (Phase 1 ADR-008) |

**背景**:
- 创始人决定仅在国内 Android 应用商店发布 (ADR-005)
- V1.3 Phase 1 = **纯本地模式** (ADR-008), 无云依赖
- 云代码 / UI 完整保留备 V2.0 启用
- 中国大陆 5 亿+ Android 用户
...
| 资源 | 状态 | 备注 |
|---|---|---|
| 软件著作权 (软著) | ❌ 未申请 | **R011 高风险 · 1-2 月 · ¥300-800** (V1.3 必需) |
| ICP 备案 | ✅ **Phase 1 不需要** | R012 暂缓 (APP 不提供 Web 服务); V2.0 启用云时再办 |
| 隐私政策 | ❌ 未写 | 中文版 (V1.3 Phase 1 写"无数据收集"声明, 简单) |
| 用户协议 | ❌ 未写 | 中文版 (如需) |
| 商店资料 (截图/图标/描述) | ❌ 未制作 | 需中文 + 各商店适应 |
| 应用商店开发者账号 | ❌ 未注册 | 各商店分别注册 |
| Release APK | ✅ 就绪 | 适用于大部分商店 |
| 签名 keystore | ⚠️ **需重生成用 V2** | PR-9 (S-14/S-25) - 防弱密码泄露 |
| V2 keystore | ✅ 已生成 (待启用) | [ADR-006](../02-architecture/04-adr/ADR-006-keystore-v2.md) |
...
1. **软著申请** (前置, 1-2 月) ✅ 必做
   ...
   - 提交 + 等颁证
2. ~~**ICP 备案评估**~~ ❌ Phase 1 不需要
3. **隐私政策 + 用户协议** (中文版) ✅ 必做
   - 写"无数据收集"声明 (纯本地不收集)
   - 用户协议模板即可
4. **V2 keystore 启用** (重签 APK 防泄露)
   - 切换 android/key.properties 指向 v2
   - 重新打包 + 验证签名
5. **商店资料准备** ✅ 必做
   - 应用截图 (5-8 张，各商店适应)
   - 应用描述 (中文)
...
**预计工作量**: **4-6 周全流程** (1-2 月软著等待 + 1-2 周上线准备)

**V1.3 完成后, V2.0 启用云时**:
- 1 行 flag 切换 + Supabase UI 默认值填回
- 启动新一轮评估 (ISSUE-037)
...
## 🆕 ☁️ V1.3 后端迁移 (ADR-007 决策 → ADR-008 撤销)
...
|---|---|
| **Issue ID** | ISSUE-036 |
| **关联决策** | ~~[ADR-007](../02-architecture/04-adr/ADR-007-r012-tcb-migration.md)~~ **撤销**, 由 [ADR-008](../02-architecture/04-adr/ADR-008-phase1-local-only-cloud-deferred.md) 取代 |
| **等级** | **P0 V1.3 关键路径** (原计划) → ⏸️ 已搁置 |
| **模块** | 后端 / BaaS 迁移 |
| **状态** | ⏸️ **已搁置 (ADR-008 决策)** |
| **撤销日期** | 2026-07-15 13:50 |

**撤销原因**:
- TCB 个人版实际为 ¥19.9/月（不是免费）
- 创始人决定 V1.3 Phase 1 纯本地模式
- 云代码 / UI 保留备 V2.0 重启

**子任务全部取消**, 待 V2.0 重新评估 (ISSUE-037)

---

### ISSUE-037 — V2.0 启用云功能评估 [占位]

| 字段 | 值 |
|---|---|
| **Issue ID** | ISSUE-037 |
| **关联决策** | [ADR-008](../02-architecture/04-adr/ADR-008-phase1-local-only-cloud-deferred.md) |
| **替代历史** | ADR-007 (撤销) |
| **等级** | P2 一般 (V2.0 候选, 不阻塞 V1.3) |
| **模块** | 云功能 / 战略 |
| **状态** | 📋 计划中 (等 V2.0 启动) |
| **决策日期** | 2026-07-15 |

**背景**:
- ADR-007 撤销后, 创始人为 V2.0 列出 4 候选云启用路径
- V1.3 Phase 1 纯本地先上架, 验证商业模式
- V2.0 (预计 2026-12+) 根据 V1.3 用户反馈 + 收入, 重新评估云提供商

**V2.0 启用云 4 候选路径**:

| 候选 | 月费 | 工作量 | 优劣 |
|---|---|---|---|
| (a) 找到真正免费的国内 BaaS | ¥0 | 6-8 周 | 期望低, 但等待可能永远 |
| (b) 接受 ¥19.9/月 TCB 个人版 | ¥19.9 | 5-7 周 (重做 ADR-007 实施) | 240 元/年可接受, 需评审收入 > 成本 |
| (c) 海外市场扩展 (保持 Supabase) | ¥0 | 2-3 周 | 但需海外发布 (iOS + Google Play) |
| (d) 永久本地化 (云架构代码转 archived) | ¥0 | 1 周 | 最终选项, 但失去差异化 |

**评估时需要新输入**:
- V1.3 国内上架后用户反馈
- V1.3 收入数据 (高级版 ¥18/月用户数)
- 国内云市场变化 (新 BaaS 出现)
- 公司化进度 (如有 V2.0 企业版需要)

**重启时**:
- 重新建 ADR-009 (或 ADR-x), 评估 4 候选
- 云代码 / UI 已完整保留, 实施工作仅为改 1 行 flag + 恢复 Supabase 默认值
...
| **报告时间** | 2026-07-18 |
| **状态** | ❌ **已撤销 (用户决定不做)** |
| **撤销时间** | 2026-07-18 12:21 (用户回复: "Bug4 不对不要执行") |
| **撤销人** | 用户 |

**用户原描述**: 在"几条改善"部分提到"客户场景数据填充", 但未给出具体需求

**用户裁决**: 2026-07-18 12:21 用户回复:
> "Bug4 不对不要执行"
...
**历史归档**: 本条目保留作为'需求变更撤销'案例, 提醒以后登记 ISSUE 时需求要明确
