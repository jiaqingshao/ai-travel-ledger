#!/usr/bin/env pwsh
# ============================================
# AI 旅行账本 - 带 Supabase 配置打包 APK (v0.2.0)
# ============================================
# 
# 用法:
...
# 
# 输出: release\v0.2.1\ai-travel-ledger-v0.2.1-release.apk (含 Supabase 配置)
# ============================================
...
Write-Host "  AI 旅行账本 v0.2.1 - Supabase 打包" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[1/5] Bump 版本号 0.2.0 -> 0.2.1..." -ForegroundColor Yellow
$pubspecPath = Join-Path $ProjectRoot 'pubspec.yaml'
$pubspec = Get-Content $pubspecPath -Raw
$newPubspec = $pubspec -replace 'version: 0\.2\.0\+2', 'version: 0.2.1+3'
if ($newPubspec -ne $pubspec) {
    Set-Content -Path $pubspecPath -Value $newPubspec -Encoding UTF8
    Write-Host "✅ pubspec.yaml 已更新到 0.2.1+3" -ForegroundColor Green
} else {
    Write-Host "⚠️  pubspec.yaml 不是 0.2.0+2, 跳过 bump" -ForegroundColor Yellow
}
Write-Host ""
...
# package
Write-Host "[5/5] 打包 release v0.2.1..." -ForegroundColor Yellow
$releaseDir = Join-Path $ProjectRoot 'release\v0.2.1'
New-Item -ItemType Directory -Force -Path $releaseDir | Out-Null

$apkSrc = Join-Path $ProjectRoot 'build\app\outputs\flutter-apk\app-release.apk'
$apkDst = Join-Path $releaseDir 'ai-travel-ledger-v0.2.1-release.apk'
Copy-Item $apkSrc $apkDst -Force
$sha1 = (Get-FileHash $apkDst -Algorithm SHA1).Hash.ToLower()
$sha1 | Out-File (Join-Path $releaseDir 'ai-travel-ledger-v0.2.1-release.apk.sha1') -Encoding ASCII

# changelog
$changelog = @"
# AI 旅行账本 v0.2.1 - Supabase 集成版

**发布日期**: 2026-07-10
**版本号**: 0.2.1+3 (versionName 0.2.1, versionCode 3)
**APK 大小**: ~25 MB
**SHA1**: $sha1

---

## 🆕 v0.2.1 新增 (Supabase 云同步)
...
228+3 = **231/231 全过** (含 v0.2.0 + v0.2.1 新增)
...
- **必须先卸载旧版** (签名不同)
- 卸载前先在 v0.2.0 APP 内配置好 Supabase 同步并上传数据
- 否则本地 Hive 数据会清空
...
1. 手机开启"未知来源应用安装"
2. 传输 `ai-travel-ledger-v0.2.1-release.apk` 到手机
3. 点击安装
4. 启动后点右上角 ☁️ 注册账号
5. 验证邮箱后登录
6. 数据自动同步到云端
...
\`\`\`bash
Get-FileHash ai-travel-ledger-v0.2.1-release.apk -Algorithm SHA1
# 预期: $sha1
\`\`\`
...
# zip
$zipPath = Join-Path $ProjectRoot 'release\ai-travel-ledger-v0.2.1.zip'
...
Write-Host "  ✅ v0.2.1 Release 完成!" -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Green
Write-Host ""
